...
function getPINs(observed) {
  ...
  for (let i = 0; i < numbers.length; i++) {
    ...
    const lower = (currentNum + 3).toString();
    const upper = (currentNum - 3).toString();
    const left = (currentNum - 1).toString();
    const right = (currentNum + 1).toString();

    if (!possibleNumbers.has(numbers[i])) {
      possibleNumbers.add(numbers[i]);
    }
    if (!possibleNumbers.has(lower) && lower <= 10) {
      possibleNumbers.add(lower.toString().slice(-1));
    }
    if (!possibleNumbers.has(upper) && upper >= 1) {
      possibleNumbers.add(upper.toString());
    }
    if (!possibleNumbers.has(left) && left >= 1) {
      possibleNumbers.add(left.toString());
    }
    if (!possibleNumbers.has(right) && right <= 10) {
      possibleNumbers.add(right.toString().slice(-1));
    }

    options.push([...possibleNumbers]);
  }
  return perm(options);
}
...
function perm(arr) {
  if (arr.length === 0) {
    ...
    const perms = [];
    const rest = perm(arr.slice(1));
    for (const x of rest) {
      for (const y of arr[0]) {
        perms.push(x + y);
      }
    }
    return perms;
