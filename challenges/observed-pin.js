...
function getPINs(observed) {
  ...
  const DFS = (chosen, choices) => {
    for (let i = 0; i < choices.length; i += 1) {
      ...
      if (chosen.length === N) {
        ...
        DFS(chosen, getAdj(parseInt(observed[chosen.length])))
      }
  ...
  DFS([], getAdj(parseInt(observed[0])))
  return output;
}
...
const getAdj = num => {
  ...
  while (row < KEYPAD.length) {
    col = 0;
    while (col < KEYPAD[row].length) {
      if (KEYPAD[row][col] === num) break;
      col++;
    }
    if (KEYPAD[row][col] === num) break;
    row++;
  }
  output.push(KEYPAD[row][col]);
